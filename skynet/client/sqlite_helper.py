...
class PickleSQLiteHelper:
    ...
    def insert_pickle(self, table_name: str, id: int, image: np.ndarray, 
                      location: List[int], time_input: List[int], pickle_obj: bytes):
        ...
        time_str = pickle.dumps(time_input)
        self.cursor.execute(f"INSERT INTO {table_name} VALUES (?, ?, ?, ?, ?)", (id, image_str, location_str, time_str, pickle_str))
        self.conn.commit()
    ...
    def retrieve_pickles_in_range(self, table_name: str, min_location: int, 
                                  max_location: int):
        self.cursor.execute(f"SELECT image, time, location, pickle FROM {table_name} WHERE location BETWEEN ? AND ?", (min_location, max_location))
...
if __name__ == '__main__':
    ...
    last_row = databasehelper.insert_pickle('test', 1, np.zeros(shape= (3,480,640)), [1, 2, 3], [1], pickle.dumps({'a': 1, 'b': 2, 'c': 3}))
    print(last_row)
